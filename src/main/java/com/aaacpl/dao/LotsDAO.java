...
public class LotsDAO {
    ...
    public Boolean insertBid(BidRequestBO bidRequestBO, Boolean isTender, int userId) throws SQLException,
            IOException {
                        ...
                        if (insertIntoLotAuditLog(connection, bidRequestBO, isBidAccepted)) {
                            ...
                            if (isBidAccepted) {
                                ...
                                if (insertIntoLiveBidLog(connection, bidRequestBO)) {
                                    if (bidRequestBO.getBidType().equals("AUTO")) {
                                        LotAutoBidMapDAO lotAutoBidMapDAO = new LotAutoBidMapDAO();
                                        int autoBidCount = lotAutoBidMapDAO.getAutoBidCount(userId, lotDTO.getId(), connection);
                                        if (autoBidCount == -1) {
                                            isProcessed = lotAutoBidMapDAO.insertIntoLotAutoBidMap(lotDTO.getId(), userId, connection);
                                        } else if (autoBidCount + 1 <= auctionDTO.getAutoBidCount()) {
                                            isProcessed = lotAutoBidMapDAO.updateAutoBidCount(lotDTO.getId(), userId, autoBidCount+1, connection);
                                        } else {
    ...
    private boolean insertIntoLotAuditLog(Connection connection,
        ...
        boolean isProcessed = false;
        try {
            String query = "INSERT INTO lot_audit_log(user_id, lot_id, bid_amt, ipAddress, clientSystemTime, localSystemTime, isAccepted) VALUES (?,?,?,?,?,?,?);";
            System.out.println("Query for lot audit insert =  "+ query);
            preparedStatement = connection
    ...
    private boolean insertIntoLiveBidLog(Connection connection,
        ...
        try {
            System.out.println("Query for live bid insert =  "+ query);
            preparedStatement = connection
                    .prepareStatement(query);
    ...
    public List<BidHistoryResponse> getBidHistoryList(int lotId, Boolean isTender)
        ...
        String query = "SELECT u.name , company_name , user_id , bid_amt , auction_id , localSystemTime, ll.isAccepted "
                ...
                + "ll.user_id=u.id AND ll.lot_id=l.id AND ll.lot_id=" + lotId+" AND a.id = "+lotDTO.getAuctionId();
        ...
        try {
            ...
            statement = connection.createStatement();
            ResultSet rs = statement.executeQuery(query);
            System.out.println("bid history query = "+query);
            while (rs.next()) {
